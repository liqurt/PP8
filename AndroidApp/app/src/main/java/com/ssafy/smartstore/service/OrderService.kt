...
class OrderService{
    ...
    // 가장 최근 주문내역 가져오는 API
//    fun getLastOrder(userId: String): LiveData<LastOrderResponse> {
//        val responseLiveData: MutableLiveData<LastOrderResponse> = MutableLiveData()
//        val lastOrderRequest: Call<LastOrderResponse> = RetrofitUtil.orderService.getLastOrder(userId)
//        var result: List<LastOrderResponse>? = null
//
//        lastOrderRequest.enqueue(object : Callback<LastOrderResponse> {
//            override fun onResponse(call: Call<LastOrderResponse>, response: Response<LastOrderResponse>) {
//                val res = response.body()
//                if(response.code() == 200){
//                    if (res != null) {
//                        responseLiveData.value = res
...
//            }
//
//            override fun onFailure(call: Call<LastOrderResponse>, t: Throwable) {
//                Log.d(TAG, t.message ?: "마지막 주문 내역 받아오는 중 통신오류")
//            }
//        })
//
//        Log.d(TAG, "onResponse before return: $result")
//        return responseLiveData
//    }
